...
@pytest.fixture
def eval_dataset(langsmith_client):
  dataset_id = "continuous-rag-eval-tutorial"
  ...
  for example in dataset_json:
    inputs.append(example["question"])
    outputs.append(example["answer"])
    metadata.append({"context": example["context"]})
